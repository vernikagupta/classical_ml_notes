...
# 📐 Math Appendix  
*Formal justification of key results used in the main text*

This appendix provides the **mathematical backbone** behind the intuitions explained earlier.  
Nothing new conceptually is introduced — this only explains **why those statements are true**.

---

## A1. Why the Gram Matrix Is Symmetric
...
Hence, the Gram matrix is **always symmetric**, regardless of the data.

---

## A2. Why the Gram Matrix Is Positive Semi-Definite

For any vector $v \in \mathbb{R}^d$:

$v^\top G v = v^\top X^\top X v = (Xv)^\top (Xv) = \|Xv\|^2 \ge 0$

Therefore:
- $v^\top G v \ge 0$ for all $v$
- All eigenvalues of $G$ are **non-negative**
...
## A3. Spectral Theorem (Real Symmetric Case)

**Theorem**

If $A \in \mathbb{R}^{d \times d}$ and $A = A^\top$, then:
...
### Consequences for $X^\top X$

Since $X^\top X$ is symmetric:
- A full eigenbasis exists
- Eigenvectors span the entire feature space
- Eigenvectors can be chosen orthogonal

This is why eigenvectors **always exist** for the Gram matrix.

---

## A4. Proof That Eigenvectors of a Symmetric Matrix Are Orthogonal
...
Using symmetry $A = A^\top$:

$v_2^\top A v_1 = (A v_2)^\top v_1 = \lambda_2 v_2^\top v_1$

But also:

$v_2^\top A v_1 = \lambda_1 v_2^\top v_1$
...
## A5. Why Eigenvalues Equal Variance After Projection

Let $v$ be a **unit eigenvector** of $X^\top X$ with eigenvalue $\lambda$.

Projected coordinate of data point $x_i$:

$z_i = x_i \cdot v$

Variance of projections:

$\text{Var}(z) = \frac{1}{n} \sum_i (x_i \cdot v)^2 = \frac{1}{n} v^\top X^\top X v$

Using the eigenvalue equation:

$= \frac{1}{n} v^\top (\lambda v) = \frac{\lambda}{n}$

Thus, **eigenvalues of $X^\top X$ are proportional to variance** along their eigenvectors.

(Scaling by $1/n$ depends on normalization convention.)

---

## A6. Why Repeated Eigenvalues Lead to Non-Unique Eigenvectors

If an eigenvalue $\lambda$ has multiplicity $k$, then:

$(A - \lambda I)v = 0$

has a **$k$-dimensional null space**.

This means:
- There are infinitely many eigenvectors
- Any orthonormal basis of that subspace is valid

Hence:
- Directions are not unique
- Only the **subspace** is meaningful

---

## A7. Why Correlation Determines Geometry

If two features satisfy:

$X_2 = c X_1$

Then all data points satisfy:
...
- Data lies in a 1D subspace
- Rank of $X$ is 1
- $X^\top X$ has exactly one non-zero eigenvalue

If features are uncorrelated:
- Covariance terms vanish
- Eigenvalues are similar
- Geometry becomes isotropic

---

## A8. Why Noise Is Direction-Agnostic

Noise $\epsilon$ is typically modeled as:
...
Hence:
- Same variance in every direction
- No preferred axis

This is why noise affects **all eigen-directions equally**.

---

## A9. Mathematical Origin of Division by Eigenvalues

Let $z_{ij} = x_i \cdot v_j$ be projected coordinates.

Variance:

$\text{Var}(z_{ij}) = \lambda_j$
...
$\text{Var}(\tilde z_{ij}) = \frac{\lambda_j}{\lambda_j} = 1$

This is the **formal origin** of division by $\sqrt{\lambda_j}$.

---

## A10. Why Small Eigenvalues Amplify Noise (Formally)

Let:

$z_{ij} = s_{ij} + \epsilon_{ij}$
...
$\tilde z_{ij} = \frac{s_{ij}}{\sqrt{\lambda_j}} + \frac{\epsilon_{ij}}{\sqrt{\lambda_j}}$

If $\lambda_j$ is small:
- $\frac{\epsilon_{ij}}{\sqrt{\lambda_j}}$ becomes large
- Noise dominates signal
...
## A11. Final Mathematical Takeaway ⭐

> The Gram matrix $X^\top X$ is symmetric and positive semi-definite, guaranteeing orthogonal eigenvectors; eigenvalues quantify variance along those directions; and any operation dividing by small eigenvalues mathematically amplifies noise.

---

*End of Math Appendix.*
