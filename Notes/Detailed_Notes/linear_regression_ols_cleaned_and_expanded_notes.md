# Linear Regression — Geometry, OLS, Conditioning & Optimization
...
Correlation gives **strength** (−1 to 1).
...
## 12. Loss Surface in β-Space

Loss is quadratic in β.

- Orthogonal features → circular contours
- Correlated features → elongated ellipses

Shape fully determined by XᵀX.

---

## 13. Ill-Conditioning & Eigenvalues

Eigenvalues of XᵀX represent curvature strength.

- Large eigenvalue → strong direction
- Small eigenvalue → flat direction

Inverse amplifies weak directions → instability.
...
Used when inverse doesn’t exist.
...
# Logistic Regression — Geometry, Likelihood, Curvature & Optimization
...
- Squared loss assumes Gaussian noise → violated
...
## 2. Logistic Model — What We Change and Why
...
- Predicting 0.01 instead of 0 → small error
- Predicting 0.01 instead of 1 → catastrophic

Squared loss treats both equally → wrong geometry.

---

## 7. Bernoulli Likelihood — Correct Noise Model
...
ℓ(β) = ∑ᵢ [ yᵢ log σ(zᵢ) + (1 − yᵢ) log(1 − σ(zᵢ)) ]
...
J(β) = − ∑ᵢ [ yᵢ log σ(zᵢ) + (1 − yᵢ) log(1 − σ(zᵢ)) ]

This is **cross-entropy loss**.

---

## 10. Gradient — Why Optimization Is Iterative
...
- Gradient depends on β nonlinearly
- No closed-form solution

Hence:

👉 **Gradient descent is mandatory**.
...
## 12. Hessian — Curvature Explained
...
- XᵀWX becomes ill-conditioned
- Flat directions appear again

Same geometry as OLS — but **weighted by confidence**.
...
## 15. Newton’s Method vs Gradient Descent
...
## 17. Unified View (Linear → Logistic)
